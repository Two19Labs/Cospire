...
export async function listAdminProcesses(): Promise<AdminProcessRow[]> {
  const supabase = await createServerSupabaseClient();
    // Only rows that are actually ARS processes. Before `courses.kind` existed
  // this listed every programme, so aptitude-prep content appeared here as
  // though it were an admission process with no rounds yet.
