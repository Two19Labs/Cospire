...
|---|---|---|---|---|---|
| Claude (test-engine chat) | `feat/test-engine` | Phase 4: slice 4.1 corrected (the attempt tables and the student's door to a mock), then 4.2 sitting the test and 4.3 scoring | `supabase/migrations/20260926103000_test_engine_attempts.sql`, `src/features/test-engine/**`, `src/app/student/mocks/**`, `src/app/student/attempts/**`, the mock grant panel in `src/features/question-bank/components/` and `src/app/admin/mocks/[id]/**`, `scripts/verify/test-engine-*` | In progress. **4.1 corrected, committed, NOT applied**: the owner runs `npx supabase db push --linked --project-ref eeeftjwvbppznsmcljnw` (the agent was refused permission to write the shared database or push the branch). Dry run 56/56. Built without the new tables: the mock grant panel on `/admin/mocks/[id]`, and the pure clock, answer and scoring modules (16 unit tests). **Blocked on the apply**: the student screens need `npm run db:types` to see `attempts` | 2026-09-26 |
